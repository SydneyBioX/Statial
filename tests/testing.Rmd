...
imageModelsNew <- getStateChanges(
  singleCellData = SCE,
  markers = markersToUse,
  Rs = c(200),
...
crossValidateInteractionsData <- imageModelsCVFormat(imageModelsNew, 
                                                     values_from = "tValue",
                                                     removeColsThresh = 0.2,
                                                     missingReplacement = 0)


crossValidateInteractionsDataModifiedd <- imageModelsNew %>%
  filter(sampleSize >= 10) %>%
  split(~ covariates + type) %>%
  bplapply(function(x) imageModelsCVFormat(x,
                                     values_from = "tValue",
                                     removeColsThresh = 0.2,
                                     missingReplacement = 0), BPPARAM = BiocParallel::MulticoreParam(workers = 1)) %>%
...
# REMOVE ALL ROWS WITH FEWER THAN 2 COLUMNS
...
#PREPROCESSING - TO BE ADDED INTO IMAGEMODELSCVFORMAT FUNCTION
