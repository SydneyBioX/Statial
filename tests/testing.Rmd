...
```{r}
colnames(colData(kerenSCE))[which(names(colData(kerenSCE)) == "CellID")] <- "ImageCellID"

colData(kerenSCE)$cellID <- rownames(colData(kerenSCE))
...
SCE <- getDistances(subset,
                         Rs = c(200),
                         whichCellTypes = c("Macrophages", "Keratin+Tumour"))
...
imageModels <- getStateChanges(
  ...
  method = "lm",
  isMixed = FALSE,
  cellTypesToModel = "Keratin+Tumour",
  nCores = 1)
...
namesVector <- unique(colData(subset)$imageID)
...
metadata(SCE)$dist200 <- metadata(SCE)$dist200 %>% mutate(condition = case_when(
