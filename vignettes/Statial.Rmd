...
In the following we will analyse breast cancer data from Keren et al. 2018. These images are stored in a `SingleCellExperiment` object. This object contains 170171 cells across 33 images and includes information on cell type and patient survival.
...
First, let's examine the same effect observed earlier with Kontextual - the distance between p53-positive keratin/tumour cells and macrophages in the context of total keratin/tumour cells for image 6 of the Keren et al. dataset. 
...
Let's take a look at the top 10 most significant gene-to-distance pairwise interactions. We can see that one of the most significant interaction involving keratin/tumour cells in image 6 
occurs between keratin/tumour cells and macrophages in image 27, where HLA Class I molecules in keratin/tumour cells are upregulated when nearby macrophage populations. 
We can examine this interaction further with the `plotStateChanges` function, showing us a clear visual correlation between macrophage density and HLA Class I expression.
...
Looking at the same top 10 most significant interactions, we can see that the results are vastly different. Running the mixed effects model appears to bring about some curious results. One of the most significant interactions occurs between B cells and CD4 T cells, where T cells are found to increase in CD20 expression when in close proximity to CD8 T cells. 
Biologically, CD20 is a highly specific ligand for B cells, and under no biologically healthy circumstances are they expressed in T cells. 

Could this potentially be an artefact of `getStateChanges`? Taking a look at an image through the `plotStateChanges` function, we indeed observe an apparent localisation between B cells and T cells.
...
```{r}
cellTypeMarkers <- c("CD3", "CD4", "CD8", "CD16", "CD56", "CD11c", "CD68", "CD45", "CD20")
...
survivalData <- kerenSCE %>%
  colData() %>%
  as.data.frame() %>%
  select(c("imageID", "Survival_days_capped", "Censored")) %>%
  distinct() %>%
  remove_rownames()
...
Here, we use the `performancePlot` function to assess the C-index from each repeat of the 3-fold cross-validation. We see that the lisaClust regions appear to capture the least amount of information that is predictive of diabetes progression status of the patients.
