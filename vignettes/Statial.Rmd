...
kerenSCE$survival = kerenSCE$Survival_days_capped
```
...
Finally, both the `SingleCellExperiment` object and the Kontextual matrix are passed into the `spicy` function, with `condition = "survival"`. The `topPairs` function can then be used to obtain the associated coefficients and p-value for the most significant pairs.
...
As we can see from the results `Dc/Mono__CD3_Cell__tcells` is the one of the most significant pairwise relationship which contributes to patient survival. That is the relationship between DC/monocytes and CD3 T cells, relative to the parent population of all T cells. We can see that there is a positive coefficient associated with this relationship, which tells us an increase in localisation of DC/monocytes and CD3 T cells relative to all T cells leads to poorer survival outcomes for patients.

The association between `Dc/Mono__CD3_Cell__tcells` and survival can also be visualised on a Kaplan-Meier curve. First, we extract survival data from the `SingleCellExperiment` object and create a survival vector. 
...
Next, we extract the Kontextual values of this relationship across all images. We then determine if DC/monocytes and CD3 T cells are relatively attracted or avoiding in each image by comparing the Kontextual value in each image to the median Kontextual value.

Finally, we plot a Kaplan-Meier curve using the `ggsurvfit` package. As shown below, when `Dc/Mono` and `CD3_Cell` are more dispersed to one another relative to the myeloid cell population, patients tend to have worse survival outcomes.

```{r, fig.width=5, fig.height=4}
# Selecting most significant relationship
survRelationship <- kontextMat[["Dc/Mono__CD3_Cell__tcells"]]
...
We can then use the `spicy` function and specify `condition = "survival"` to run survival analysis.
