...
stateChanges <- calculateStateChanges(
  cells = kerenSCE,
  type = "distances",
  image = "6",
  from = "Keratin_Tumour",
...
ggplot() + geom_line(data = stateChanges, aes(cumsum(marker %in% cellTypeMarkers) , y = cumsum(!marker %in% cellTypeMarkers)), colour = "blue") +
  geom_line(data = stateChangesCorrected, aes(cumsum(marker %in% cellTypeMarkers) , y = cumsum(!marker %in% cellTypeMarkers)), colour = "red")


ggplot() + geom_line(data = stateChanges, aes(cumsum(marker %in% cellTypeMarkers) , y = cumsum(!marker %in% cellTypeMarkers)), colour = "blue") +
  geom_line(data = stateChangesCorrected, aes(cumsum(marker %in% cellTypeMarkers) , y = cumsum(!marker %in% cellTypeMarkers)), colour = "red") + xlim(0,100) + ylim(0,1000)
...
kontextMat = prepMatrix(kerenKontextual, type = "Kontextual")
...
StatialOutput <- prepMatrix(stateChanges,
                                         values_from = "tValue",
